 ...
 *
 * @param dbPath - Path to the DuckDB database generated by `pvduck`.
 * @param outputPath - Optional path to write the output JSON file to. If not
 * 	provided, a new timestamped file is created in the data directory.
 */
async function aggregate(dbPath: string, outputPath?: string): Promise<void> {
	const [filename, authorsPath, worksPath, authorshipsPath, notablesPath] =
		await Promise.all([
			outputPath ?? getNewPath("aggregate", "json"),
	...
	if (!authorsPath || !worksPath || !authorshipsPath || !notablesPath) {
...
program
	.command("aggregate <dbPath> [outputPath]")
	.description("Aggregate the data in the database")
	.action(async (dbPath: string, outputPath?: string) => {
